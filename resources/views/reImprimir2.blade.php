...
<table class="table">
  <thead>
    <tr>
      <th scope="col">RE-IMPRIMIR NÚMERO DE ATENCIÓN</th>
      
    </tr>
  </thead>
  <tbody>
  	@foreach($ValidacionNoAgenRe_Imprimir as $coleccion2)
    <tr>
      <th scope="row"><h1>{{$coleccion2->numero_atencion}}</h1></th>
       @php
...
function imprimir(cod_tpdf,jslinea, js_nro_mision)
  {
    ...
    f1.writeline("ReImpresión  : <?php date_default_timezone_set('America/Santiago');
        echo $hoy = date('d/m/Y g:ia');; ?>");
    f1.writeline("--------------------------------");
    f1.writeline("                                ");
    f1.writeline("                                ");
