...
<div class="card-header py-3">
  <p class="text-right">
  <a href="{{route('BloqueoHorarioFijo.create') }}" class="btn btn-primary ">Agregar Horario</a></p>
              <h5 class="m-0 font-weight-bold text-primary">Bloqueo Horario Fijo</h5>
            </div>
...
<table class="table table-hover" id="dataTable"  >
  ...
  <tbody>
    ...
    $id=1;
    @endphp
  	@foreach($bloqueo_horario as $bloqueohorario)
    
    <tr>
      <th scope="row">{{$id++}}</th>     
      <td>{{$bloqueohorario->hora_inicio_bloq_horario}}</td>
      <td>{{$bloqueohorario->hora_fin_bloq_horario}}</td>
      <td><form action="{{route('BloqueoHorarioFijo.destroy',$bloqueohorario->id_bloq_horario)}}" method="POST">
      @csrf
      @method('DELETE')
