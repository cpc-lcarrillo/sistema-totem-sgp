...
<form action="{{ route('Users.update', $listarusuarios->id_usuario) }}" method="POST">
  ...
  <div class="form-row">
    ...
    <div class="form-group col-md-6">
      <label >Nombre</label>
      <input type="text" class="form-control" name="nombre" value="{{$listarusuarios->nombre}}"  id="nombre"  placeholder="Nombre" required>
      <span class="text-danger">{{$errors->first('nombre')}}</span>
    </div>
    <div class="form-group col-md-6">
      <label>Apellido</label>
      <input type="text" class="form-control" name="apellido" value="{{$listarusuarios->apellido}}" id="apellido " placeholder="apellido" required>
